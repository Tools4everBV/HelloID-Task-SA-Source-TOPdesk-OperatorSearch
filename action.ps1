try {
    ...
    if ([String]::IsNullOrEmpty($searchValue) -eq $true) {
        ...
        $splatParams = @{
            Uri         = "$($topdeskBaseUrl)/tas/api/operators?page_size=2&query=employeeNumber=sw=$searchValue,surName=sw=$searchValue,firstName=sw=$searchValue,email=sw=$searchValue,networkLoginName=sw=$searchValue"
            Method      = 'GET'
            Verbose     = $false
            Headers     = $Headers
